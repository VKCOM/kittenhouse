...
func (state *parseConfigState) servers(hosts []string) (destination.ServersStr, []destination.Server, error) {
	...
	if withWeightCount > 0 && len(servers) != withWeightCount {
		return "", nil, fmt.Errorf("Cannot use servers with and without weights at the same time")
	}
...
func parseHostWithWeight(arg string) (host string, weight uint32, err error) {
	...
	argCropped := arg[1:]
	starPos := strings.IndexByte(argCropped, '*')

	asteriskFound := starPos > 0 && starPos < len(argCropped)-1
	...
	weightStr := argCropped[starPos+1:]

	weight64, err := strconv.ParseUint(weightStr, 10, 32)
	if err != nil {
	...
	if weight == 0 {
		return "", 0,  errors.New("Weight 0 is not valid")
	}

	return argCropped[0:starPos], weight, nil
}

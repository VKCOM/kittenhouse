...
func (s *sender) sendFile(relFilename string, off ackOff) (newOff ackOff, fullyDelivered bool, err error) {
	...
	if err != nil {
		...
		return off, false, errors.Wrap(err, `open relFilename fail`)
	}
	defer fp.Close()
	...
	if off.Table == "" {
		...
		off.Table, off.RowBinary, err = s.determineTableName(fp, relFilename)
		if err != nil {
			return off, false, errors.Wrap(err, `determineTableName fail`)
		}
	}

	if _, err = fp.Seek(off.Offset, os.SEEK_SET); err != nil {
		return off, false, errors.Wrap(err, `fd seek fail`)
	}
	...
	bytesRead, fullyDelivered, err := s.readLinesIntoBuf(fp, maxSendSize, off.RowBinary, relFilename)
	if err != nil {
		return off, false, errors.Wrap(err, `readLinesIntoBuf fail`)
	}
	...
	if err := clickhouse.Flush(s.dst, off.Table, s.buf, off.RowBinary); err != nil {
		...
		if shouldRetry {
			return off, false, errors.Wrap(err, `ch.Flush fail`)
		}
	}
...
func (s *sender) determineTableName(fp io.Reader, relFilename string) (tableName string, rowBinary bool, err error) {
	...
	for {
		...
		ln, ok := s.trimCrc(ln[0:len(ln)-1], relFilename)
		if !ok || len(ln) <= 1 || ln[0] != '#' {
			continue
		}
...
func (s *sender) trimCrc(ln []byte, relFilename string) (cleanLn []byte, ok bool) {
	if len(ln) < 8 {
	...
	if !bytes.Equal(actualCrc, crcStr) {
		...
		log.Printf("Got broken line (invalid CRC32, expected '%s', got '%s'. line len: %d) table '%s': '%s'", crcStr, actualCrc, l, relFilename, ln)
		return nil, false
	}
...
func (s *sender) writeLineIntoBuf(ln []byte, rowBinary bool, relFilename string) {
	if len(ln) > 0 && ln[0] == '#' { // ignore header
		return
	}

	ln, ok := s.trimCrc(ln, relFilename)
	if !ok {
		return
	}
...
func (s *sender) readLinesIntoBuf(r io.Reader, maxSendSize int64, rowBinary bool, relFilename string) (bytesRead int64, fullyDelivered bool, err error) {
	...
	for {
		...
		for _, c := range s.readBuf[0:n] {
			if c == '\n' {
				s.writeLineIntoBuf(s.curLine, rowBinary, relFilename)

...
func IsSyntaxError(err error) bool {
	...
	return strings.Contains(e.msg, `Cannot parse input:`) || strings.Contains(e.msg, `Type mismatch`) || strings.Contains(e.msg, `Cannot read all data`) || strings.Contains(e.msg, `Cannot read array from text`)
}
...
func checkHostAlive(srv destination.ServerHostPort) error {
	...
	code, res, err := queryServer(time.Now().Add(time.Minute), srv, `SELECT 42 FORMAT TabSeparated`, &params)
	if err != nil {
		return err
	}
...
func flush(dst *destination.Setting, table string, body []byte, rowBinary bool, compression bool) error {
	...
	reqUrl := fmt.Sprintf("http://%s/?input_format_values_interpret_expressions=0&%squery=%s", srv, compressionArgs, queryPrefix)

	resp, err := httpClient.Post(reqUrl, "application/x-www-form-urlencoded", bytes.NewReader(body))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if bytes.Contains(bodyText, nginxBytes) {
			log.Printf("For table %s got nginx response: %s", table, bodyText)
			dst.TempDisableHost(srv, checkHostAlive)
		}
